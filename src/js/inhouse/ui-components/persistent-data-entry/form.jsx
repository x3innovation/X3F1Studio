...
module.exports = React.createClass({
	...
	****************************************** */
	...
	saveUiToGoogle: function() {
		...
		this.model.gFields.set(fieldIndex, this.model.fieldData);
	}, 
	...
	setRefSelectOptions: function() {
		...
		if (refs !== []) {
			...
			$('#ref-name-select').material_select(function(){
				this.onRefTypeChanged($('#ref-name-dropdown .select-dropdown').val());
			}.bind(this));
		}
	...
	onFieldSelected: function(data) {
		...
		for (i = 0, len = fields.length; i<len; i += 1) {
			if (fields[i].name === selectedField) {
				this.model.fieldData = this.model.gFields.get(i);
				break;
			}
		}
		this.updateUi();
