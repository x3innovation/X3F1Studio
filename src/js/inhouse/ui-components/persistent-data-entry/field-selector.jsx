...
module.exports = React.createClass ({
	...
	****************************************** */
	...
	onGapiFileLoaded: function(doc) {
		...
		this.model.gFields.addEventListener(gapi.drive.realtime.EventType.VALUES_SET, this.updateUi);
		this.updateUi();
		this.gapiLoaded = true;
	}, 

	updateUi: function(e) {
		this.model.fields = this.model.gFields.asArray();
		this.initializeTable();
		this.selectField();
	}, 

	saveNewField: function(newFieldName) {
		if (!this.model.gFields) {
			return false;
		}
		var fields = this.model.gFields.asArray();
		...
		this.model.gFields.push(newField);
	}, 
	...
	onAddBtnClick: function(e) {
		...
		$('.field-cell').each(function(index, element){
			if ($(this).text().indexOf(NEW_FIELD_NAME) === 0) {
				digitsList.push($(this).text().substring(NEW_FIELD_NAME.length));
			}
		});
			...
			<div>
				...
				<div>
					<a id = 'field-add-btn' onClick = {this.onAddBtnClick} 
					   className = {'field-selector-header-btn btn-floating waves-effect waves-light '+Configs.App.ADD_BUTTON_COLOR}>
						<i className = 'mdi-content-add btn-icon field-selector-header-btn-icon'></i></a>
				</div>
				<div>
					<a id = 'field-delete-btn' 
					   className = 'delete-tooltipped field-selector-header-btn btn-floating waves-effect waves-light red'>
