 ...
 * @const
 */
rtclient.REALTIME_MIMETYPE = 'application/dmx';
...
rtclient.Authorizer = function(options) {
	...
	ball.userId = rtclient.params['userId'];
 ...
 */
rtclient.createRealtimeFile = function(title, folder, mimeType, callback) {
	console.log("begin rtclient.createRealtimeFile()");
...
rtclient.RealtimeLoader = function(options) {
	...
	this.defaultTitle = rtclient.getOption(options, 'defaultTitle', 'New Model');
...
rtclient.RealtimeLoader.prototype.start = function() {
	console.log("begin rtclient.RealtimeLoader.prototype.start()");
...
rtclient.RealtimeLoader.prototype.load = function() {
	console.log("begin rtclient.RealtimeLoader.prototype.load()");
	var action = rtclient.params['action'];
	var projectFolderId = rtclient.params['projectFolderId'];
	if (action == 'new') {
		if (projectFolderId != null) {
			this.createNewFileAndRedirect(projectFolderId);
		}
	}
 ...
 */
rtclient.RealtimeLoader.prototype.createNewFileAndRedirect = function(parentFolderId) {
	var _this = this;

	dog.getFolder(parentFolderId, function(folder) {
		rtclient.createRealtimeFile(ball.DMX_DEFAULTNAME, folder, ball.DMX_MIMETYPE, function(file) {
			console.log("new file has been created: " + file.id);
			if (file.id) {
				//window.location.replace('./#fileIds=' + file.id + '&userId=' + ball.userId);
				_this.redirectTo([file.id], ball.userId);
			}
