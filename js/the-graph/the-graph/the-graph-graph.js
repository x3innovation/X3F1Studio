...
//                this.addEdge(newEdge);
        ...
        renderPreviewEdge: function(event) {
            ...
            console.log("this.props.app.state.y: " + this.props.app.state.y);
            this.setState({
        ...
        portInfo: {},
        getPorts: function(processName, component) {
            ...
            if (!ports) {
                ...
                for (i = 0, len = component.outports.length; i < len; i++) {
                    ...
                    outports[port.name] = {
                        label: port.name,
                        type: port.type,
                        x: TheGraph.nodeWidth,
                        y: 32 + i * 10 //TheGraph.nodeSize / (len+1) * (i+1)
                    };
                }
        ...
        getNodeOutport: function(processName, portName, route, component) {
            var ports = this.getPorts(processName, component);
        ...
        getNodeInport: function(processName, portName, route, component) {
            var ports = this.getPorts(processName, component);
        ...
        render: function() {
            ...
            var edges = graph.edges.map(function(edge) {
                ...
                // Label
                var label = source.metadata.label + " " + edge.from.port.toUpperCase() + " -> " + edge.to.port.toUpperCase() + " " + target.metadata.label;
                var key = edge.from.node + "() " + edge.from.port.toUpperCase() + " -> " + edge.to.port.toUpperCase() + " " + edge.to.node + "()";
                ...
                return TheGraph.Edge({
                    ...
                    edge: edge,
                    app: self.props.app,
                    sX: source.metadata.x + TheGraph.nodeWidth,
            ...
            var inports = Object.keys(graph.inports).map(function(key) {
                ...
                var expEdge = {
                    ...
                    sX: expNode.x + TheGraph.nodeWidth,
