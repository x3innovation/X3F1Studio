(function(context) {"use strict";
	...
	TheGraph.findMinMax = function(graph, nodes) {
		...
		for (var i = 0; i < len; i++) {
			...
			if (node.metadata.x > maxX) {
				maxX = node.metadata.x;
			}
			if (node.metadata.y > maxY) {
				maxY = node.metadata.y;
			}
		}
	...
	TheGraph.TextBG = React.createClass({
		componentDidMount : function() {
			console.log("TheGraph.TextBG componentDidMount");
			//if (this.onNodeSelection) {
			this.getDOMNode().addEventListener("tap", this.onClicked, true);
			this.getDOMNode().addEventListener("mouseenter", this.onMouseEnter, true);
			//}
		},
		onClicked : function(event) {
			console.log("TheGraph.TextBG onClicked");
			// Don't tap app (unselect)
			event.stopPropagation();


			// document.fire("nodeLabelClicked", {
			// x : this.props.x,
			// y : this.props.y,
			// });
		...
		render : function() {
			...
			// children : text,
			// position: "absolute",
			// left:65px;top:4px; ;display:table-cell;padding:10px;border:2px solid red;background-color:white;opacity:0.5;font-family:Verdana;font-size:20px;white-space: pre;word-wrap: normal;
			// }
			//)
			);
		}
