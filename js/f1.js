    ...
    var loopDMXFilesCallback = function() {
        ...
        for (var i = 0; i < dmxFileIds.length; i++) {
            ball.getDMXModel(dmxFileIds[i], function(doc) {
                ...
                jsonRoot.Data.push(node);

                if (dmxType == 'persisted') {
                    jsonRoot.UpdatePersistenceEvent.push({
                        _name: "Update" + dmxName.charAt(0).toUpperCase() + dmxName.slice(1),
                        _typeId: doc.UpdatePersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.CreatePersistenceEvent.push({
                        _name: "Create" + dmxName.charAt(0).toUpperCase() + dmxName.slice(1),
                        _typeId: doc.CreatePersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.RemovePersistenceEvent.push({
                        _name: "Remove" + dmxName.charAt(0).toUpperCase() + dmxName.slice(1),
                        _typeId: doc.RemovePersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.UpdatedPersistenceEvent.push({
                        _name: dmxName.charAt(0).toUpperCase() + dmxName.slice(1) + "Updated",
                        _typeId: doc.UpdatedPersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.CreatedPersistenceEvent.push({
                        _name: dmxName.charAt(0).toUpperCase() + dmxName.slice(1) + "Created",
                        _typeId: doc.CreatedPersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.RemovedPersistenceEvent.push({
                        _name: dmxName.charAt(0).toUpperCase() + dmxName.slice(1) + "Removed",
                        _typeId: doc.RemovedPersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.RejectUpdatePersistenceEvent.push({
                        _name: "RejectUpdate" + dmxName.charAt(0).toUpperCase() + dmxName.slice(1),
                        _typeId: doc.RejectUpdatePersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.RejectCreatePersistenceEvent.push({
                        _name: "RejectCreate" + dmxName.charAt(0).toUpperCase() + dmxName.slice(1),
                        _typeId: doc.RejectCreatePersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                    jsonRoot.RejectRemovePersistenceEvent.push({
                        _name: "RejectRemove" + dmxName.charAt(0).toUpperCase() + dmxName.slice(1),
                        _typeId: doc.RejectRemovePersistenceEventTypeId,
                        _persistedData: dmxName
                    });
                }
...
f1.generateXML = function() {
    ...
    f1.generateAllEnumXML(function(node) {
        ...
        //2. get all DMX
        console.log("calling f1.generateAllDataXML()");
        f1.generateAllDataXML(function(node) {
            console.log("f1.generateAllDataXML()");
            jsonRoot.FlxSchema.Data = node.Data;

            jsonRoot.FlxSchema.UpdatePersistenceEvent = node.UpdatePersistenceEvent;
            jsonRoot.FlxSchema.CreatePersistenceEvent = node.CreatePersistenceEvent;
            jsonRoot.FlxSchema.RemovePersistenceEvent = node.RemovePersistenceEvent;

            jsonRoot.FlxSchema.UpdatedPersistenceEvent = node.UpdatedPersistenceEvent;
            jsonRoot.FlxSchema.CreatedPersistenceEvent = node.CreatedPersistenceEvent;
            jsonRoot.FlxSchema.RemovedPersistenceEvent = node.RemovedPersistenceEvent;

            jsonRoot.FlxSchema.RejectUpdatePersistenceEvent = node.RejectUpdatePersistenceEvent;
            jsonRoot.FlxSchema.RejectCreatePersistenceEvent = node.RejectCreatePersistenceEvent;
            jsonRoot.FlxSchema.RejectRemovePersistenceEvent = node.RejectRemovePersistenceEvent;
