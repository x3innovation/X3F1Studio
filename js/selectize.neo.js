...
( function(root, factory) {
		...
		Sifter.prototype.iterator = function(object, callback) {
			var iterator;
			if (is_array(object)) {
				iterator = Array.prototype.forEach ||
				function(callback) {
...
( function(root, factory) {
	...
	}(this, function($, Sifter, MicroPlugin) {'use strict';
		...
		MicroEvent.prototype = {
			...
			trigger : function(event /* , args... */) {
				console.log("trigger is called on event " + event);
				this._events = this._events || {};
				if ( event in this._events === false)
					return;
		...
		MicroEvent.mixin = function(destObject) {
			var props = ['on', 'off', 'trigger', 'hasEvent'];
		...
		$.extend(Selectize.prototype, {
			...
			onMouseDown : function(e) {
				...
				console.log("selectize: onMouseDown()");
			...
			addOptionQuiet : function(data) {
				var i, n, optgroup, value, self = this;

				if ($.isArray(data)) {
					for ( i = 0, n = data.length; i < n; i++) {
						self.addOptionQuiet(data[i]);
					}
					return;
				}
			 ...
			 * @param {string} value
			 */
			addItemsQuiet : function(values) {
				var items = $.isArray(values) ? values : [values];
				for (var i = 0, n = items.length; i < n; i++) {
					this.isPending = (i < n - 1);
					this.addItemQuiet(items[i]);
				}
			},
			 ...
			 * @param {string} value
			 */
			addItemQuiet : function(value) {
				this.addItem1(value, true);
			},
			...
			addItem : function(value) {
				debounce_events(this, ['change'], function() {
					this.addItem1(value, false);
				});
			},

			addItem1 : function(value, disableTrigger) {
				//console.log("in addItemQuiet(" + value + ")");
				...
				if (self.isSetup) {
					...
					if (disableTrigger != true) {
						//console.log("trigger item_add");
						self.trigger('item_add', value, $item);
					} else {
						//console.log("not trigger item_add");
					}
					if (disableTrigger) {
						self.updateOriginalInputQuiet();
					} else {
						self.updateOriginalInput();
					}
				}
			 ...
			 */
			updateOriginalInputQuiet : function() {
			...
			open : function() {
				...
				console.log("open: this.events['open'].length = " + self);
				if (self.hasEvent('open')) {//self.hasEvent('open')
					//call onOpen
					self.trigger('open');
				} else {
					self.openDropdown();
